...
double eval_real_function(double *coefficients, int degree, double x);
double eval_real_d_function(double *coefficients, int degree, double x);
double find_real_root(double *coefficients, int degree, double x0);
int sovle_real_polynomial_equation(double *coefficients, double **solutions, int degree);
...
double *solve_system_equation(int num_of_sol, double **coefficients);
...
double eval_real_function(double *coefficients, int degree, double x)
{
    ...
    {
        f += coefficients[i] * pow(x, degree - i);
    }
    return f;
}
...
double eval_real_d_function(double *coefficients, int degree, double x)
{
    ...
    for (int i = 0; i < degree; i++)
    {
        df += (degree - i) * coefficients[i] * pow(x, degree - 1 - i);
    }
    return df;
}
...
double find_real_root(double *coefficients, int degree, double x0)
{
    ...
    for (int i = 0; i < 900; i++)
    {
        f = eval_real_function(coefficients, degree, x0);
        df = eval_real_d_function(coefficients, degree, x0);
        x1 = x0 - (f / df);
        x0 = x1;
...
/*
  This module sovles all real solutions of any polynomial equation with positive integer degree

  #Note:
    This module return value = 0 ---> no real solutions found
    This module return value = 1 ---> at least 1 solution found
    ...

  #Example usage:

    double coefficients[] = {-8, 1, 9, -5, 0};
    int degree = sizeof(coefficients) / sizeof(coefficients[0]) - 1;
    double *solutions =(double *)malloc((degree + 1) * sizeof(double));

    int numofsolutions = sovle_real_polynomial_equation(coefficients, &solutions, degree);
...
{
    // find min solution
    double xrmin = find_real_root(coefficients, degree, -1000);
    (*solutions)[0] = xrmin;

    // find max solution
    double xrmax = find_real_root(coefficients, degree, 1000);

    // find all solutions
    double step = fabs(xrmax - xrmin - 0.001) / 200;
    int num_of_sol = 1;
    if (xrmin != NAN && xrmax != NAN)
    { // if real solutions are found
        for (double t = xrmin + 0.001; t < xrmax; t += step)
        {
            if (eval_real_function(coefficients, degree, t) * eval_real_function(coefficients, degree, t + step) <= 0)
            {
                (*solutions)[num_of_sol++] = find_real_root(coefficients, degree, t);
            }
        }
        return num_of_sol;
    }
...
double **coefficients =(double **)malloc(2 * sizeof(double *));
for(int row = 0; row < 2 + 1; row++) coefficients[row] =(double *)malloc(3 * sizeof(double));
