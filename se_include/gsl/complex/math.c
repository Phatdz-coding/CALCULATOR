...
{                               /* z = sin(a) */
  double R = GSL_REAL (a);
  double gsl_imag_var = GSL_IMAG (a);
  ...
  if (gsl_imag_var == 0.0) 
    ...
    {
      GSL_SET_COMPLEX (&z, sin (R) * cosh (gsl_imag_var), cos (R) * sinh (gsl_imag_var));
    }
...
{                               /* z = cos(a) */
  double R = GSL_REAL (a), gsl_imag_var = GSL_IMAG (a);
  ...
  if (gsl_imag_var == 0.0) 
    ...
    {
      GSL_SET_COMPLEX (&z, cos (R) * cosh (gsl_imag_var), sin (R) * sinh (-gsl_imag_var));
    }
...
{                               /* z = tan(a) */
  ...
  if (fabs (gsl_imag_var) < 1)
    {
      double D = pow (cos (R), 2.0) + pow (sinh (gsl_imag_var), 2.0);

      GSL_SET_COMPLEX (&z, 0.5 * sin (2 * R) / D, 0.5 * sinh (2 * gsl_imag_var) / D);
    }
  else
    {
      double D = pow (cos (R), 2.0) + pow (sinh (gsl_imag_var), 2.0);
      double F = 1 + pow(cos (R)/sinh (gsl_imag_var), 2.0);

      GSL_SET_COMPLEX (&z, 0.5 * sin (2 * R) / D, 1 / (tanh (gsl_imag_var) * F));
    }
...
{                               /* z = arcsin(a) */
  ...
  if (gsl_imag_var == 0)
    ...
    {
      double x = fabs (R), y = fabs (gsl_imag_var);
      ...
      GSL_SET_COMPLEX (&z, (R >= 0) ? real : -real, (gsl_imag_var >= 0) ? imag : -imag);
    }
...
{                               /* z = arccos(a) */
  ...
  if (gsl_imag_var == 0)
  ...
  else
    {
      ...
      GSL_SET_COMPLEX (&z, (R >= 0) ? real : M_PI - real, (gsl_imag_var >= 0) ? -imag : imag);
    }
...
{                               /* z = arctan(a) */
  ...
  if (gsl_imag_var == 0)
  ...
  else
    {
      ...
      double r = hypot (R, gsl_imag_var);

      double imag;

      double u = 2 * gsl_imag_var / (1 + r * r);
      ...
      if (fabs (u) < 0.1)
        ...
        {
          double A = hypot (R, gsl_imag_var + 1);
          double B = hypot (R, gsl_imag_var - 1);
          imag = 0.5 * log (A / B);
        }
...
{                               /* z = sinh(a) */
  ...
  gsl_complex z;
  GSL_SET_COMPLEX (&z, sinh (R) * cos (gsl_imag_var), cosh (R) * sin (gsl_imag_var));
  return z;
}
...
{                               /* z = cosh(a) */
  ...
  gsl_complex z;
  GSL_SET_COMPLEX (&z, cosh (R) * cos (gsl_imag_var), sinh (R) * sin (gsl_imag_var));
  return z;
}
...
{                               /* z = tanh(a) */
  ...
  if (fabs(R) < 1.0) 
    {
      double D = pow (cos (gsl_imag_var), 2.0) + pow (sinh (R), 2.0);
      
      GSL_SET_COMPLEX (&z, sinh (R) * cosh (R) / D, 0.5 * sin (2 * gsl_imag_var) / D);
    }
  else
    {
      double D = pow (cos (gsl_imag_var), 2.0) + pow (sinh (R), 2.0);
      double F = 1 + pow (cos (gsl_imag_var) / sinh (R), 2.0);

      GSL_SET_COMPLEX (&z, 1.0 / (tanh (R) * F), 0.5 * sin (2 * gsl_imag_var) / D);
    }
